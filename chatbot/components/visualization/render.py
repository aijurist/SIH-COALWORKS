...
from chatbot.components.visualization.schema import (
    AggregationType,
    BarMode,
    ChartType,
    Filter,
...
from chatbot.components.visualization.transform import transform
...
def _is_datetime_like_column(s: pd.Series) -> bool:
    ...
    try:
        ...
        try:
            pd.to_datetime(s, dayfirst=True, format='%Y-%m-%d')
            return True
        except Exception:
            return False
